...
module.exports = async function handler(req, res) {
  ...
  const { contractData, question, history = [] } = req.body || {};
  ...
  //   2. PDF upload → extract text, prepend to question
  //   3. Image      → vision content block + text question
  ...
  if (fileType === "photo" && fileBase64) {
    ...
    userContent = [
      ...
      {
        type: "text",
        text: `This is a photo of a funeral contract. Please analyze it carefully and answer the following question:\n\n${question.trim()}`,
      },
    ];
  } else if (fileType === "pdf" && fileBase64) {
    // PDF mode — extract text first, then treat as rich form context
    const pdfText = await extractPdfText(fileBase64);
    if (pdfText && pdfText.trim().length > 50) {
      userContent = `The following is the extracted text from an uploaded funeral contract PDF. Please analyze it and answer the question below.\n\n--- CONTRACT TEXT START ---\n${pdfText.slice(0, 12000)}\n--- CONTRACT TEXT END ---\n\nQuestion: ${question.trim()}`;
    } else {
      // PDF parse failed or empty — fall back with a note
      userContent = `The user uploaded a PDF funeral contract (${contractData?.fileName || "unknown file"}) but the text could not be extracted automatically. Please let them know and ask them to describe the key details or switch to the manual entry form.\n\nTheir question was: ${question.trim()}`;
    }
  } else {
